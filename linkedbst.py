...
class LinkedBST(AbstractCollection):
    ...
    def isBalanced(self):
        ...
        :return:
        '''
        return 2 * log(self._size, 2) > self.height()
    ...
    def rangeFind(self, low, high):
        ...
        def make_lst(root, k1, k2):
            if root:
                if k1 < root.data:
                    make_lst(root.left, k1, k2)
                if k1 <= root.data <= k2:
                    lst.append(root.data)
                if k2 > root.data:
                    make_lst(root.right, k1, k2)

        make_lst(self._root, low, high)
        return lst
